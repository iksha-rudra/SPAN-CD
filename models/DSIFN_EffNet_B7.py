...
from segmentation_models_pytorch.encoders import efficient_net_encoders

PATH = '../../../checkpoints_trans_1/checkpoint.pth'

class EfficientNet_B7_base(nn.Module):
    def __init__(self, in_c, backbone_type = 'random'):
        super(EfficientNet_B7_base,self).__init__()
        self.in_c = in_c
        self.feature_indices = (1,2,3,5,7)
        
        if backbone_type == 'random':
            self.encoder = efficientnet_b7(weights=None)
        elif backbone_type == 'imagenet':
            self.encoder = efficientnet_b7(weights='IMAGENET1K_V1')
        elif backbone_type == 'pretrained':
            self.encoder = efficientnet_b7(weights=None)
        
        # print(self.encoder.features) 
           
        self.encoder.features[0][0] = nn.Conv2d(13, 64, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)    
...
class DSIFN_EfficientNet_B7(nn.Module):
    """Some Information about DSIFN_ResNet"""
    def __init__(self, in_c, out_c,  backbone_type = 'random', finetune = True):
        super(DSIFN_EfficientNet_B7, self).__init__()
        
        model1 = EfficientNet_B7_base(in_c, backbone_type)
...
if __name__ == '__main__':
    ...
    model = DSIFN_EfficientNet_B7(in_c=13, out_c=2, backbone_type = 'random', finetune = False)
    out = model(x1, x2)
