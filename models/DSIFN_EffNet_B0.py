...
class EfficientNet_B0_base(nn.Module):
    def __init__(self, in_c, backbone_type = 'random'):
        ...
        if backbone_type == 'random':
            ...
            self.encoder = efficientnet_b0(weights='IMAGENET1K_V1')
...
class DSIFN_EfficientNet_B0(nn.Module):
    """Some Information about DSIFN_ResNet"""
    def __init__(self, in_c, out_c,  backbone_type = 'random', finetune = True):
        super(DSIFN_EfficientNet_B0, self).__init__()
        
        model1 = EfficientNet_B0_base(in_c, backbone_type)
...
if __name__ == '__main__':
    ...
    model = DSIFN_EfficientNet_B0(in_c=13, out_c=2, backbone_type = 'random', finetune = False)
    out = model(x1, x2)
