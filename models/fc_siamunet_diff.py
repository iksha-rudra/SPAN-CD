...
class SiamUNet_diff(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        """Forward method."""
        # Stage 1
        x11 = self.do11(F.gelu(self.bn11(self.conv11(x1))))
        x12_1 = self.do12(F.gelu(self.bn12(self.conv12(x11))))
        x1p = F.max_pool2d(x12_1, kernel_size=2, stride=2)


        # Stage 2
        x21 = self.do21(F.gelu(self.bn21(self.conv21(x1p))))
        x22_1 = self.do22(F.gelu(self.bn22(self.conv22(x21))))
        x2p = F.max_pool2d(x22_1, kernel_size=2, stride=2)

        # Stage 3
        x31 = self.do31(F.gelu(self.bn31(self.conv31(x2p))))
        x32 = self.do32(F.gelu(self.bn32(self.conv32(x31))))
        x33_1 = self.do33(F.gelu(self.bn33(self.conv33(x32))))
        x3p = F.max_pool2d(x33_1, kernel_size=2, stride=2)

        # Stage 4
        x41 = self.do41(F.gelu(self.bn41(self.conv41(x3p))))
        x42 = self.do42(F.gelu(self.bn42(self.conv42(x41))))
        x43_1 = self.do43(F.gelu(self.bn43(self.conv43(x42))))
        x4p = F.max_pool2d(x43_1, kernel_size=2, stride=2)

        ####################################################
        # Stage 1
        x11 = self.do11(F.gelu(self.bn11(self.conv11(x2))))
        x12_2 = self.do12(F.gelu(self.bn12(self.conv12(x11))))
        x1p = F.max_pool2d(x12_2, kernel_size=2, stride=2)


        # Stage 2
        x21 = self.do21(F.gelu(self.bn21(self.conv21(x1p))))
        x22_2 = self.do22(F.gelu(self.bn22(self.conv22(x21))))
        x2p = F.max_pool2d(x22_2, kernel_size=2, stride=2)

        # Stage 3
        x31 = self.do31(F.gelu(self.bn31(self.conv31(x2p))))
        x32 = self.do32(F.gelu(self.bn32(self.conv32(x31))))
        x33_2 = self.do33(F.gelu(self.bn33(self.conv33(x32))))
        x3p = F.max_pool2d(x33_2, kernel_size=2, stride=2)

        # Stage 4
        x41 = self.do41(F.gelu(self.bn41(self.conv41(x3p))))
        x42 = self.do42(F.gelu(self.bn42(self.conv42(x41))))
        x43_2 = self.do43(F.gelu(self.bn43(self.conv43(x42))))
