...
class EfficientNet_B1_base(nn.Module):
    def __init__(self, in_c, backbone_type = 'random'):
        super(EfficientNet_B1_base,self).__init__()
        self.in_c = in_c
        ...
        if backbone_type == 'random':
            ...
            self.encoder = efficientnet_b1(weights='IMAGENET1K_V1')
        elif backbone_type == 'pretrained':
            self.encoder = efficientnet_b1(weights=None)
            
        # print(self.encoder)
           
        # print(len(self.encoder.features))
        self.encoder.features[0][0] = nn.Conv2d(13, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False) 
           
        # print(self.encoder.features[0][0])
...
class DSIFN_EfficientNet_B1(nn.Module):
    """Some Information about DSIFN_ResNet"""
    def __init__(self, in_c, out_c,  backbone_type = 'random', finetune = True):
        super(DSIFN_EfficientNet_B1, self).__init__()
        
        model1 = EfficientNet_B1_base(in_c, backbone_type)
...
if __name__ == '__main__':
    ...
    model = DSIFN_EfficientNet_B1(in_c=13, out_c=2, backbone_type = 'random', finetune = False)
    out = model(x1, x2)
    
    print(out.shape)
    
    # model = EfficientNet_B1_base(in_c=13)
    # out = model(x1)