...
def get_segmentation_model(backbone, feature_indices, feature_channels):
    ...
    Returns:
        [type]: [description]
    """
    model = SegmentationEncoder(backbone, feature_indices, feature_channels, diff=False)

    # for param in model.parameters():
    #     param.requires_grad = False

    unet = UNet(model, feature_channels, 2, bilinear=True, concat_mult=2, dropout_rate=0.3)
    # unet = UNetSmall(model, feature_channels, 1, bilinear=True, concat_mult=1)
    return unet

class SiamSegment(torch.nn.Module):

    def __init__(self, backbone_type = 'random'):
        super().__init__()
        
        if backbone_type == 'random':
            # backbone = resnet.resnet18(weights=None)
            backbone = efficientnet_b4(weights=None)
            backbone.features[0][0] = nn.Conv2d(13, 48, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)    
            # backbone.conv1 = torch.nn.Conv2d(13, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        elif backbone_type == 'imagenet':
            backbone = resnet.resnet18(weights='IMAGENET1K_V1')
            backbone.conv1 = torch.nn.Conv2d(13, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        ...
        feature_indices = (1, 2, 3, 5 ,7)
        feature_channels = (24, 32, 56, 160, 448)
    ...
    def __init__(self, encoder, feature_channels, n_classes, concat_mult=2, bilinear=True, dropout_rate=0.5):
        ...
        for i in range(0, len(feature_channels) - 1):
            ...
            setattr(self, 'batchnorm%d' % i,
                    SwitchNorm2d(feature_channels[i] * concat_mult))
            # setattr(self, 'cbam%d' % i,
                    # CBAM(feature_channels[i] * concat_mult))
        self.outc = OutConv(feature_channels[0] * concat_mult, n_classes)
        self.encoder = encoder
...
class UNetSmall(nn.Module):
    def __init__(self, encoder, feature_channels, n_classes, concat_mult=2, bilinear=True):
        ...
        self.bn = SwitchNorm2d(feature_channels[0])
...
if __name__ == '__main__':
    ...
    model = SiamSegment(backbone_type='imagenet')
    output = model(torch.zeros((1, 12, 96, 96)), torch.zeros((1, 12, 96, 96)))
