...
class ToTensor(object):
    def __call__(self, pre_img):
        return F.to_tensor(pre_img)


class Normalize(object):
    def __init__(self, pre_img_mean, post_img_mean, 
                 pre_img_std, post_img_std, 
                 inplace=False):
        
        self.pre_img_mean = pre_img_mean
        self.pre_img_std = pre_img_std
        self.inplace = inplace

    def __call__(self, pre_tensor, post_tensor, mask):
        return F.normalize(pre_tensor, self.pre_img_mean, self.pre_img_std, self.inplace)
...
class ToGray(object):
    ...
    def __call__(self, pre_img):
        return F.to_grayscale(pre_img, self.output_channels)
...
#     def __call__(self, pre_img):
#         return F.noise(pre_img, self.mode)
...
class GaussianBlur(object):
    ...
    def __call__(self, pre_img):
        return F.gaussian_blur(pre_img, self.kernel_size)
...
class RandomNoise(object):
    ...
    def __init__(self, mode="gaussian", percent=0.002):
    ...
    def __call__(self, pre_img):
        return F.noise(pre_img, self.mode, self.percent)
...
class RandomBrightness(object):
    ...
    def __call__(self, pre_img):
        return F.adjust_brightness(pre_img, self.value)
...
class RandomContrast(object):
    ...
    def __call__(self, pre_img):
        return F.adjust_contrast(pre_img, self.factor)
...
class RandomShift(object):
    ...
    def __call__(self, pre_img):
        height, width = pre_img.shape[0:2]
        ...
        return F.shift(pre_img, top, left)
...
class RandomCrop(object):
    ...
    def __call__(self, pre_img):
        ...
        return F.crop(pre_img, top, left, th, tw)
...
class RandomFlip(object):
    ...
    def __call__(self, pre_img):
        if random.random() < self.p:
            ...
            return F.flip(pre_img, flip_code)
        return pre_img
