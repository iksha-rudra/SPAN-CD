...
class OSCD_Dataset(Dataset):
    ...
    def get_list_masks(self, mask):
        list = []
        
        img = mask.numpy()
        img = img[0,:,:]

        mask1 = cv2.resize(img, (128, 128), cv2.INTER_CUBIC)
        mask2 = cv2.resize(img, (64, 64), cv2.INTER_CUBIC)
        mask3 = cv2.resize(img, (32, 32), cv2.INTER_CUBIC)
        mask4 = cv2.resize(img, (16, 16), cv2.INTER_CUBIC)
        mask5 = cv2.resize(img, (8, 8), cv2.INTER_CUBIC)
        
        list.append(mask)
        list.append(torch.from_numpy(np.expand_dims(mask1,axis=0)))
        list.append(torch.from_numpy(np.expand_dims(mask2,axis=0)))
        list.append(torch.from_numpy(np.expand_dims(mask3,axis=0)))
        list.append(torch.from_numpy(np.expand_dims(mask4,axis=0)))
        list.append(torch.from_numpy(np.expand_dims(mask5,axis=0)))
    ...
    def __getitem__(self, idx):
        ...
        list_imgs = self.get_list_masks(label)
        
        if self.normalize:
            I1 = (I1 - I1.mean()) / I1.std()
            I2 = (I2 - I2.mean()) / I2.std()
        
        sample = {'I1': I1, 'I2': I2, 'label': label, 'fname': im_name, 'list': list_imgs}
    ...
    def image_normalize_quantile(self,image, first=1, last=99):
        ...
        for i in range(C):
            ...
            a, b = np.percentile(image[:, :, i], (first, last))
            image[:, :, i] = np.clip(image[:, :, i], a, b)
            # image[:, :, i] = (image[:, :, i] - a) / (b - a)
        return image
    ...
    def read_sentinel_img_leq20(self, path):
        ...
        I = np.stack((b,g,r,nir,ir1,ir2,ir3,nir2,swir2,swir3),axis=2).astype('float')
    ...
    def reshape_for_torch(self, I):
        ...
        return torch.from_numpy(out.astype(np.float64))
    
    def get_res10_bands(self, path):
        """Read cropped Sentinel-2 image: RGB and NIR bands."""
        im_name = os.listdir(path)[0][:-7]
        
        img60 = io.imread(path + im_name + "B01.tif")
        s = (img60.shape[0] * 6 , img60.shape[1] * 6)
        
        
        r = self.adjust_shape(io.imread(path + im_name + "B04.tif"),s)
        g = self.adjust_shape(io.imread(path + im_name + "B03.tif"),s)
        b = self.adjust_shape(io.imread(path + im_name + "B02.tif"),s)
        nir = self.adjust_shape(io.imread(path + im_name + "B08.tif"),s)
        
        I = np.stack((b,g,r,nir),axis=2).astype('float')
        return I
    
    def get_res20_bands(self, path):
        """Read cropped Sentinel-2 image: bands with resolution less than or equals to 20m."""
        im_name = os.listdir(path)[0][:-7]
        
        img60 = io.imread(path + im_name + "B01.tif")
        s = (img60.shape[0] * 3 , img60.shape[1] * 3)
        
        ir1 = self.adjust_shape(io.imread(path + im_name + "B05.tif"),s)
        ir2 = self.adjust_shape(io.imread(path + im_name + "B06.tif"),s)
        ir3 = self.adjust_shape(io.imread(path + im_name + "B07.tif"),s)
        nir2 = self.adjust_shape(io.imread(path + im_name + "B8A.tif"),s)
        swir2 = self.adjust_shape(io.imread(path + im_name + "B11.tif"),s)
        swir3 = self.adjust_shape(io.imread(path + im_name + "B12.tif"),s)
        
        I = np.stack((ir1,ir2,ir3,nir2,swir2,swir3),axis=2).astype('float')

        return I
    
    def get_res60_bands(self, path):
        """Read cropped Sentinel-2 image: all bands."""
        im_name = os.listdir(path)[0][:-7]
        
        uv = io.imread(path + im_name + "B01.tif")
        wv = io.imread(path + im_name + "B09.tif")
        swirc = io.imread(path + im_name + "B10.tif")
        
        I = np.stack((uv,wv,swirc),axis=2).astype('float')
