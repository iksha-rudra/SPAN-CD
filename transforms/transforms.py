...
class Normalize(object):
    def __init__(self, pre_img_mean, post_img_mean, 
        ...
        self.post_img_mean = post_img_mean
        self.post_img_std = post_img_std
    ...
    def __call__(self, pre_tensor, post_tensor, mask):
        return F.normalize(pre_tensor, self.pre_img_mean, self.pre_img_std, self.inplace), \
               F.normalize(post_tensor, self.post_img_mean, self.post_img_std, self.inplace), \
               mask
...
class ToGray(object):
    ...
    def __call__(self, pre_img, post_img, mask):
        return F.to_grayscale(pre_img, self.output_channels), F.to_grayscale(post_img, self.output_channels), mask
...
class RandomBrightness(object):
    ...
    def __call__(self, pre_img, post_img, mask):
        return F.adjust_brightness(pre_img, self.value), F.adjust_brightness(post_img, self.value), mask
...
class RandomResizedCrop(object):
    ...
    def __call__(self, pre_img, post_img, mask):
        ...
        new_mask = F.crop(mask, top, left, th, tw)
        new_mask = F.resize(new_mask, self.target_size, interpolation=Image.NEAREST)
        new_mask = np.expand_dims(new_mask, axis=2)

        return pre_img, post_img, new_mask
